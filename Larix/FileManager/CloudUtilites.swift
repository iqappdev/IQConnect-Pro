...
class CloudUtilites: NSObject, PHPhotoLibraryChangeObserver, NSFilePresenter {
    ...
    let larixAlbumTitle = "Larix Broadcaster"
    var saveFileUrl: URL?
    ...
    func movePhoto(fileUrl: URL, to storage: LarixSettings.RecordStorage) {
        saveFileUrl = fileUrl
        switch storage {
        case .local:
            return
    ...
    func moveVideo(fileUrl: URL, to storage: LarixSettings.RecordStorage) {
        switch storage {
        case .local:
            return
        case .photoLibrary:
    ...
    private func initPhotoLibrary() {
        ...
        if let id = LarixSettings.sharedInstance.photoAlbumId {
        ...
        if collection == nil {
            ...
            if result.count > 0 {
                larixCollection = result
                LarixSettings.sharedInstance.photoAlbumId = result.firstObject!.localIdentifier
            }
        }
            ...
            if let folderRequest = changeInstance.changeDetails(for: allCollections!) {
                ...
                for newObject in added {
                    if let collection = newObject as? PHAssetCollection, collection.localizedTitle == larixAlbumTitle {
                        ...
                        LarixSettings.sharedInstance.photoAlbumId = id
