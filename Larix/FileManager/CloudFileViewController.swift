...
//            attr.append(" ,")
//        }
//        DDLogDebug("Metadata attributes: \(attr)" )
            ...
            let deleteItem = UIContextualAction(style: .destructive, title: "Delete") { (action, view, postAction) in
                ...
                if let url = metadata.value(forAttribute: NSMetadataItemURLKey) as? URL {
                    do {
                        ...
                        Toast(text: NSLocalizedString("File deleted", comment: ""), duration: Delay.short).show()
                        postAction(true)
                    } catch {
                        DDLogError("Failed to remove: \(error.localizedDescription)" )
                        Toast(text: NSLocalizedString("Failed to delete file", comment: ""), duration: Delay.short).show()
    ...
    func deleteSelected() {
        ...
        DispatchQueue.main.async {
            ...
            Toast(text: NSLocalizedString("Deleted \(count) files", comment: ""), duration: Delay.short).show()
        }
    }
    ...
    func downloadSelected() {
        ...
        DispatchQueue.main.async {
            Toast(text: NSLocalizedString("Downloading \(count) files", comment: ""), duration: Delay.short).show()
    ...
    func moveSelected() {
        ...
        DispatchQueue.main.async {
            Toast(text: NSLocalizedString("Moved \(count) files", comment: ""), duration: Delay.short).show()
            self.tableView.setEditing(false, animated: true)
            self.navigationController?.setToolbarHidden(true, animated: true)
        }
    }
    ...
    func purgeSelected() {
        ...
        DispatchQueue.main.async {
            Toast(text: NSLocalizedString("Purged \(count) files", comment: ""), duration: Delay.short).show()
            self.tableView.setEditing(false, animated: true)
            self.navigationController?.setToolbarHidden(true, animated: true)
        }
    }
    ...
    func performOnSelected(_ action: (NSMetadataItem)->Bool) -> Int {
        ...
        for indexPath in selected {
            ...
            guard let metadata = self.cloudQuery?.result(at: idx) as? NSMetadataItem else { continue }
            if action(metadata) {
                actionCount += 1
            }
        }
