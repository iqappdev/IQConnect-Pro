...
class LarixSettings: LarixSettingsKeys {
    
    static let sharedInstance = LarixSettings() // Singleton pattern in Swift
    ...
    private let VideoBitrates: [String:Int] = ["2160":4500, "1080":2500, "720":1500, "540":1500, "480":800, "360":500, "288":500, "144":300]
    
    var videoBitrate: Int {
        var bitrate = UserDefaults.standard.integer(forKey: video_bitrate_key)
        if bitrate == video_bitrate_auto {
            let height = UserDefaults.standard.string(forKey: video_resolution_key)
            if let recommended = VideoBitrates[height ?? String(video_resolution_hd)] {
                bitrate = recommended
...
//                }
    ...
    var fps: Double {
        get {
            var framerate = UserDefaults.standard.double(forKey: video_framerate_key)
            if framerate == 0 {
        ...
        set {
            UserDefaults.standard.set(String(newValue), forKey: video_framerate_key)
        }
    }
    
    private var keyFrameIntervalDuration: Double {
        var interval = UserDefaults.standard.integer(forKey: video_keyframe_key)
        if interval == 0 {
    ...
    var cameraPosition: AVCaptureDevice.Position {
        let location = UserDefaults.standard.string(forKey: camera_location_key) ?? camera_location_back
    ...
    var portrait: Bool {
        let orientation = UserDefaults.standard.string(forKey: video_orientation_key)
...
//            let typeStr = UserDefaults.standard.string(forKey: video_codec_type_key) ?? video_codec_type_h264
//            if typeStr == video_codec_type_hevc, isHEVCSupported {
//                return kCMVideoCodecType_HEVC
//            }
            let typeStr = SharedManager.sharedInstance.getVideoCodec()
            if typeStr == "HEVC", isHEVCSupported {
                return kCMVideoCodecType_HEVC
            }
        }
    ...
    private var profileLevel: CFString {
        if #available (iOS 11.0, *) {
            if videoCodecType == kCMVideoCodecType_HEVC {
                let hevc = UserDefaults.standard.string(forKey: hevc_profile_key) ?? main
                switch hevc {
                case main10:
        ...
        let avc = UserDefaults.standard.string(forKey: avc_profile_key) ?? baseline
        switch avc {
        case high:
    ...
    var videoConfig: VideoConfig {
        ...
        print("fps:\(fps), videoBitrate=\(videoBitrate)")
...
//        Toast(text: message).show()
    ...
    private var channelCount: Int {
        var channels = UserDefaults.standard.integer(forKey: audio_channels_key)
        if channels == 0 {
    ...
    private var sampleRate: Double {
        var samplerate = UserDefaults.standard.double(forKey: audio_samplerate_key)
    ...
    var canBroadcast: Bool = true
    var record: Bool = true
    
    var postprocess: Bool {
//        let value = UserDefaults.standard.bool(forKey: core_image_key)
//        if value == false {
//            return value
//        } else {
//            return canPostprocess
//        }
        return SharedManager.sharedInstance.getWatermarkStatus() == 0
    }
    ...
    var liveRotation: Bool {
        let rotation = UserDefaults.standard.string(forKey: live_rotation_key) ?? live_rotation_on
        return rotation == live_rotation_on
    }
    
    var resolution: CMVideoDimensions {
        get {
            var res = UserDefaults.standard.integer(forKey: video_resolution_key)
            if res == 0 {
        ...
        set {
            UserDefaults.standard.set(String(newValue.height), forKey: video_resolution_key)
        }
    }
    
    var videoStabilizationMode: AVCaptureVideoStabilizationMode {
        if let mode = UserDefaults.standard.string(forKey: stabilization_mode_key) {
            switch mode {
    ...
    var preferredInput: AVAudioSession.Port? {
        if let port = UserDefaults.standard.string(forKey: session_port_key) {
            switch port {
    ...
    var audioBitrate: Int {
        get {
            let bitrate = UserDefaults.standard.integer(forKey: audio_bitrate_key)
        ...
        set {
            UserDefaults.standard.set(newValue, forKey: audio_bitrate_key)
        }
    }
    
    var abrMode: Int {
        return UserDefaults.standard.integer(forKey: abr_mode_key)
    }
    
    var adaptiveFps: Bool {
        return UserDefaults.standard.bool(forKey: adaptive_fps_key)
    }
    ...
    var photoAlbumId: String? {
        get {
            return UserDefaults.standard.string(forKey: photo_album_id)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: photo_album_id)
        }
    }
    
    public var recordStorage: RecordStorage {
        let val = UserDefaults.standard.string(forKey: record_storage_key) ?? RecordStorage.local.rawValue
        return RecordStorage.init(rawValue: val) ?? .local
    }
    
    public var recordDuration: Int {
        let duratonMin =  UserDefaults.standard.integer(forKey: record_duration_key)
        return duratonMin * 60
    }
    
    public var snapshotFormat: SnapshotFormat {
        let val = UserDefaults.standard.string(forKey: snapshot_format_key) ?? default_snapshot_format
    ...
    public var multiCamMode: MultiCamMode {
        if #available(iOS 13.0, *) {
            ...
            let val = UserDefaults.standard.string(forKey: multi_cam_key) ?? MultiCamMode.off.rawValue
    ...
    func getDefaultBackCamera(probe:((AVCaptureDevice, CMVideoDimensions, Double) -> Bool)? = nil) -> AVCaptureDevice? {
        ...
        let typeStr = UserDefaults.standard.string(forKey: camera_type_key)
        ...
        if typeStr != camera_type_default {
            ...
            UserDefaults.standard.set(camera_type_default, forKey: camera_type_key)
        }
        return virtualCamera
    }
    ...
    public var volumeKeysCapture: Bool {
        ...
        let val = UserDefaults.standard.string(forKey: volume_keys_capture_key) ?? default_volume_keys
    ...
    func groveVideoConfig() -> [URLQueryItem] {
        ...
        if let orientation = UserDefaults.standard.string(forKey: video_orientation_key) {
            config.append(URLQueryItem(name: "enc[vid][orientation]", value: orientation))
        }
        
        if let typeStr = UserDefaults.standard.string(forKey: camera_type_key) {
        ...
        let rotation = UserDefaults.standard.bool(forKey: core_image_key)
        if rotation {
            let live_rotation = UserDefaults.standard.string(forKey: live_rotation_key) ?? ""
        ...
        let bitrate = UserDefaults.standard.integer(forKey: video_bitrate_key)
    ...
    func groveAudioConfig() -> [URLQueryItem] {
        var config: [URLQueryItem] = []
        let channels = UserDefaults.standard.string(forKey: audio_channels_key) ?? "1"
        config.append(URLQueryItem(name: "enc[aud][channels]", value: channels))

        let bitrate = UserDefaults.standard.string(forKey: audio_bitrate_key) ?? "0"
        config.append(URLQueryItem(name: "enc[aud][bitrate]", value: bitrate))

        let samplerate = UserDefaults.standard.string(forKey: audio_samplerate_key) ?? "0"
    ...
    func groveRecordConfig() -> [URLQueryItem] {
        ...
        if record {
            let duratonMin = UserDefaults.standard.string(forKey: record_duration_key) ?? "0"
