...
which Larix will use for stream transmission and for re-sending packets, combined.
...
"""
    ...
    let unsupportedAuth = NSLocalizedString("Larix doesn't support this type of RTMP authorization. Please use rtmpauth URL parameter or other \"Target type\" for authorization.", comment: "")
    ...
    override func viewDidLoad() {
        ...
        if let talbackSection = addTalkbackSection(uri: connUri) {
            form +++ talbackSection
        }
...
//        }
    ...
    func shareView( row: ButtonRow, ip: String) -> UIViewController {
        ...
        if self is ConnectionEditorViewController {
            title = NSLocalizedString("Larix Broadcaster SRT settings", comment: "")
            description = String.localizedStringWithFormat("The following URL(s) can be used to connect to Larix Broadcaster to receive SRT publication from %@:", name)
        } else {
            title = NSLocalizedString("Larix Broadcaster SRT talkback settings", comment: "")
            description = String.localizedStringWithFormat("The following URL(s) can be used to connect to Larix Broadcaster to send the talkback stream via SRT to %@:", name)
        }
    ...
    func validateAuth(uri connUri: ConnectionUri ) -> String? {
        ...
        if let login = form.rowBy(tag: "login")?.baseValue as? String {
            ...
            guard let password = form.rowBy(tag: "password")?.baseValue as? String else {
                return NSLocalizedString("Larix doesn't support empty password. Please fill in both \"Login\" and \"Password\" input fields to define stream credentials.", comment: "")
            }
            conn.username = login
            ...
            if let _ = form.rowBy(tag: "password")?.baseValue {
                if connUri.isRtmp, !isAuth {
                    ...
                    return NSLocalizedString("Larix doesn't support empty username. Please fill in both \"Login\" and \"Password\" input fields to define stream credentials.", comment: "")
                }
            }
        }
    ...
    @objc func updateAction(barButtonItem: UIBarButtonItem) {
        if let section = form.sectionBy(tag: "connection_editor"), let conn = connection {
            ...
            if update {
                if save() {
                    DispatchQueue.main.async {
                        Toast(text: String.localizedStringWithFormat(NSLocalizedString("Updated: \"%@\"", comment: ""), conn.name)).show()
                    }
                }
            } else {
                DispatchQueue.main.async {
                    Toast(text: String.localizedStringWithFormat(NSLocalizedString("\"%@\" is not changed", comment: ""), conn.name)).show()
                }
            }
        }
    }
