...
AVCapturePhotoCaptureDelegate, StreamerEngineDelegate {
    ...
    internal var blackFrame: CVPixelBuffer?
    ...
    private func setupAudioSession() throws {
        ...
        if let inputs = audioSession.availableInputs, let preferredInput = LarixSettings.sharedInstance.preferredInput {
            for input in inputs {
                DDLogVerbose("\(input)")
    ...
    internal func setCameraParams(camera: AVCaptureDevice) -> AVCaptureDevice.Format? {
        ...
        if activeFormat == nil {
            self.videoConfig!.fps = LarixSettings.sharedInstance.video_framerate_def
        ...
        SwiftTryCatch.try({
            ...
            camera.activeVideoMinFrameDuration = CMTimeMake(value: 1, timescale: Int32(self.videoConfig!.fps))
            camera.activeVideoMaxFrameDuration = CMTimeMake(value: 1, timescale: Int32(self.videoConfig!.fps))
            self.currentFps = self.videoConfig!.fps
        }, catch: { (error) in
        ...
        camera.videoZoomFactor = getInitZoomFactor(forDevice: camera)
    ...
    internal func setVideoStabilizationMode(connection: AVCaptureConnection, camera: AVCaptureDevice) {
        ...
        let mode = LarixSettings.sharedInstance.videoStabilizationMode
    ...
    func setupStereo(input: AVAudioSessionPortDescription) -> Bool {
        ...
        let position = LarixSettings.sharedInstance.cameraPosition
        if position == .front {
    ...
    func changeBitrate(newBitrate: Int32) {
        ...
        if LarixSettings.sharedInstance.adaptiveFps {
    ...
    func getInitZoomFactor(forDevice camera: AVCaptureDevice) -> CGFloat {
        ...
        if #available(iOS 13.0, *) {
            if camera.isVirtualDevice == true {
                ...
                if subDevices.count > 1 {
                    let mainCameraIndex = subDevices.firstIndex { (dev) -> Bool in
                        dev.deviceType == .builtInWideAngleCamera
                    }
                    if mainCameraIndex != nil && mainCameraIndex! > 0 {
                        let zoom = camera.virtualDeviceSwitchOverVideoZoomFactors[mainCameraIndex! - 1]
    ...
    func findFormat(fps: inout Double, camera: AVCaptureDevice?) -> AVCaptureDevice.Format? {
        guard let formats = camera?.formats else {return nil}
...
//            }
    ...
    internal func outputBlackFrame(withPresentationTime time: CMTime) {
        if (blackFrame == nil) {
